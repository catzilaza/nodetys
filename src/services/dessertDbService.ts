...
export async function connectToDatabase1() {
  ...
  const dessertCollection: mongoDB.Collection = db.collection(
    `${process.env.DESSERT_COLLECTION_NAME}`
  );
  const userCollection: mongoDB.Collection = db.collection(
    `${process.env.USER_COLLECTION_NAME}`
  );

  collections.desserts = dessertCollection;
  collections.users = userCollection;

  console.log(
    `Successfully connected to database: ${db.databaseName} and collection: ${dessertCollection.collectionName}
